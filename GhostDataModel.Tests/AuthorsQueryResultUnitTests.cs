...
public class AuthorsQueryResultUnitTests
{
    string testJson =
        @"
{
...
";
    ...
    {
        var authorsQueryResult = JsonSerializer.Deserialize<AuthorsQueryResult>(
            testJson,
            new JsonSerializerOptions() { }
        );
        Assert.NotNull(authorsQueryResult);
        TestContext.Progress.WriteLine(
            $"{authorsQueryResult} {JsonSerializer.Serialize<AuthorsQueryResult>(authorsQueryResult)}"
        );
        Assert.NotNull(authorsQueryResult.Authors);
        Assert.That(authorsQueryResult.Authors.Count, Is.EqualTo(1));

        var author = authorsQueryResult.Authors[0];
