...
";
    ...
    string testJsonWithTagsAndAuthors =
        @"
{
    ""slug"": ""welcome-short"",
...
";
    ...
    {
        var post = JsonSerializer.Deserialize<Post>(testJson, new JsonSerializerOptions() { });
        Assert.NotNull(post);
        TestContext.Progress.WriteLine($"{post} {JsonSerializer.Serialize<Post>(post)}");

        Assert.That(post.Slug, Is.EqualTo("welcome-short"));
        Assert.That(post.Id, Is.EqualTo("5ddc9141c35e7700383b2937"));
        Assert.That(post.Uuid, Is.EqualTo("a5aa9bd8-ea31-415c-b452-3040dae1e730"));
        Assert.That(post.Title, Is.EqualTo("Welcome"));
        Assert.That(post.Html, Is.EqualTo("<p>👋 Welcome, it's great to have you here.</p>"));
        Assert.That(post.CommentId, Is.EqualTo("5ddc9141c35e7700383b2937"));
        Assert.That(
            post.FeatureImage,
            Is.EqualTo("https://static.ghost.org/v3.0.0/images/welcome-to-ghost.png")
        );
        Assert.That(post.Featured, Is.EqualTo(false));
        Assert.That(post.Visibility, Is.EqualTo("public"));
        // Assert.That(
        //     post.CreatedAt,
        //     Is.EqualTo(new DateTimeOffset("2019-11-26T02:43:13.000+00:00"))
        // );
        // Assert.That(
        //     post.UpdatedAt,
        //     Is.EqualTo(new DateTimeOffset("2019-11-26T02:44:17.000+00:00"))
        // );
        // Assert.That(
        //     post.PublishedAt,
        //     Is.EqualTo(new DateTimeOffset("2019-11-26T02:44:17.000+00:00"))
        // );
        Assert.That(post.Url, Is.EqualTo("https://docs.ghost.io/welcome-short/"));
        Assert.That(post.Excerpt, Is.EqualTo("👋 Welcome, it's great to have you here."));
        Assert.That(post.ReadingTime, Is.EqualTo(0));
        Assert.That(post.Access, Is.EqualTo(true));
    }

    [Test]
    public void TestJsonWithTagsAndAuthors()
    {
        var post = JsonSerializer.Deserialize<Post>(
            testJsonWithTagsAndAuthors,
